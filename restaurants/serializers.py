...
class RestaurantReadSerializer(serializers.ModelSerializer):
    ...
    def get_description(self, obj: Restaurant):
        return get_translated_field(
            self.context["request"], obj.description_ar, obj.description_en
        )

    def get_categories(self, obj: Restaurant):
        categories = MenuCategoryReadSerializer(
            obj.categories, many=True, context=self.context
        ).data
        return [
...
class MenuItemBaseSerializer(serializers.ModelSerializer):
    ...
    def get_name(self, obj):
        return get_translated_field(
            self.context.get("request"), obj.name_ar, obj.name_en
        )

    def get_description(self, obj):
        return get_translated_field(
            self.context.get("request"), obj.description_ar, obj.description_en
        )
    ...
    def get_menu_item(self, obj: OrderItem):
        return {
            ...
            "name": get_translated_field(
                self.context["request"], obj.menu_item.name_ar, obj.menu_item.name_en
            ),
        }
...
class OrderReadSerializer(serializers.ModelSerializer):
    ...
    def get_restaurant(self, obj: Order):
        return {
            ...
            "name": get_translated_field(
                self.context["request"], obj.restaurant.name_ar, obj.restaurant.name_en
            ),
        }
...
class OrderWriteSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        address = validated_data.get("address_current")
        if not address or address.strip() == "":
            address = client.address
            if not address or address.strip() == "":
                raise serializers.ValidationError(
                    {"address": "No address provided and client has no address."}
                )
            validated_data["address_current"] = address
