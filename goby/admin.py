...
def re_display_name(name , length = 20) : 
    if not name : 
        return "Anon"
...
class CustomAdminURLWidget(forms.MultiWidget):
    def __init__(self, attrs=None):
        widgets = [
            forms.TextInput(attrs={'class': 'url-input'}),
            forms.URLInput(attrs={'class': 'url-preview'}),
        ]
    ...
    def format_output(self, rendered_widgets):
        ...
        return format_html('<div class="url-widget">{0}{1}</div>', rendered_widgets[0], rendered_widgets[1])
